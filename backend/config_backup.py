...
import os
from d    # Base de datos IAMC (SQL Server) - Ahora es la base de datos principal
    IAMC_DB_CONFIG = {
        'server': 'localhost',
        'database': 'IAMC',
        'username': 'sa',
    ...
    # Configuración heredada (ya no se usa)
    # Base de datos externa (SQL Server) - Para compatibilidad con código legacy
    DB_CONFIG = {
    ...
    # Base de datos IAMC (SQL Server) - Ahora es la base de datos principaledelta
...
class Config:
    ...
    # Configuración de SQLAlchemy - Cambio a IAMC como BD principal
    ...
    # Base de datos IAMC (SQL Server) - Ahora es la base de datos principal
    IAMC_DB_CONFIG = {
    ...
    def obtener_conexion_iamc():
        """Establece una conexión con la base de datos IAMC (SQL Server) - Principal"""
        try:
            return pyodbc.connect(
                f"DRIVER={{ODBC Driver 17 for SQL Server}};"
                f"SERVER={Config.IAMC_DB_CONFIG['server']};"
                f"DATABASE={Config.IAMC_DB_CONFIG['database']};"
                f"UID={Config.IAMC_DB_CONFIG['username']};"
                f"PWD={Config.IAMC_DB_CONFIG['password']};"
                f"TrustServerCertificate=yes;"
                f"Encrypt=yes"
            )
        except Exception as e:
            import logging
            logging.error(f"Error connecting to IAMC SQL Server: {str(e)}")
            raise

    @staticmethod
    def obtener_conexion_remota():
        """Establece una conexión con la base de datos SQL Server (legacy)"""
        try:
            return pyodbc.connect(
                f"DRIVER={{ODBC Driver 17 for SQL Server}};"
